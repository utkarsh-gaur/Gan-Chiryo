...
function SingleRecordDetails() {
  ...
  const state = location.state || {}; // Provide a default empty object if state is null
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const updatedRecord = await updateRecord({
        documentID: state.id || "default-id", // Use a default ID if state.id is undefined
        analysisResult: text,
        kanbanRecords: "",
      });
  ...
  return (
    <div className="flex flex-wrap gap-[26px]">
      ...
      <FileUploadModal
        ...
        filename={filename}
      />
      <RecordDetailsHeader recordName={state.recordName || "Untitled Record"} />{" "}
      {/* Use a default record name if state.recordName is undefined */}
                ...
                <div className="flex w-full flex-col px-6 py-4 text-white">
                  <div>
                    ...
                    <div className="space-y-2">
                      <ReactMarkdown>
                        {analysisResult || "No analysis available yet."}
                      </ReactMarkdown>{" "}
                      {/* Provide default text if analysisResult is null */}
                    </div>
                  </div>
